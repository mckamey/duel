...
o("");c.firstChild;)h.appendChild(c.firstChild);c=h}f(e,c)}}A(a);J(a);if(a.nodeType===11&&a.childNodes.length===1)a=a.firstChild;return a}var n=u.document;v.prototype.toString=function(){return this.value};var k;k=function(a,b,d,c,f){var e;switch(j(a)){case 1:e=a(b,d,c);break;case 2:e=a[0]||"";switch(e){case "$for":var g=a[1]||{};e=[""];if(g.hasOwnProperty("count")){var i=g.count;if(l(i))i=i(b,d,c);if(g.hasOwnProperty("data")){g=g.data;if(l(g))g=g(b,d,c)}else g=b;for(b=0;b<i;b++)r(e,s(a,g,b,i,f))}else{if(g.hasOwnProperty("in")){var h=
g["in"];if(l(h))h=h(b,d,c);if(j(h)===3){g=[];for(i in h)h.hasOwnProperty(i)&&g.push({key:i,value:h[i]})}else g=h}else{g=g.each;if(l(g))g=g(b,d,c)}if(j(g)===2){b=0;for(d=g.length;b<d;b++)r(e,s(a,g[b],b,d,f))}else e=s(a,g,0,1,f)}e=e;break;case "$xor":e=w(a,b,d,c,f);break;case "$if":e=w(["$xor",a],b,d,c,f);break;case "$call":f=a[1];if(!f||!f.view)e=null;else{e=k(f.view,b,d,c);i=k(f.data,b,d,c);g=k(f.index,b,d,c);b=k(f.count,b,d,c);d={};c=2;for(h=a.length;c<h;c++){var D=a[c];if((f=D[1]||{})&&f.name)d[f.name]=
